...
/* #define TEST */
#define TEST_HOME_DIR "/home/rodde" /* Change to this to your home dir. */
...
const char *const OPERATION_DESCRIPTOR_SHOW_TAG_ENTRY_LIST
    = "show_tag_entry_list";
...
static void jump_to_previous_directory()
{
    ...
    if (entry == NULL) {
        /* The tags file is empty: */
        next_path = get_current_working_directory();
        entry = dt_entry_alloc(PREV_TAG_NAME, next_path);
    }
...
static void list_tags_only(dt_entry_list* list)
{
    dt_entry* e;
    size_t i;
    puts(OPERATION_DESCRIPTOR_SHOW_TAG_ENTRY_LIST);
...
static char* get_format_str(size_t max_tag_length)
{
    ...
    char* fmt;
    char buffer[20]; /* Tag length maximum 1000 chars + zero terminator */
...
static const char* strip_leading_tilde(const char *const path)
{
    const char* home_dir;
    size_t home_dir_len;
    size_t path_len;
    ...
    /* We allow only absolute paths, or the one relative */
    ...
    rv = malloc(home_dir_len + path_len + 1);
    memcpy(rv, home_dir, home_dir_len);
    rv[home_dir_len] = '/';
    memcpy(rv + home_dir_len, path + 1, path_len);
    return rv;
}
...
static void switch_directory(const char* const tag)
{
    ...
    if (e == NULL) {
        ...
        update_previous_directory(FLAG_UPDATE_PREVIOUS, cwd);
        printf("%s\n%s",
               OPERATION_SWITCH_DIRECTORY,
               strip_leading_tilde(dt_entry_get_dir(e)));
    }
}
...
static void test_strip_tilde()
{
    const char* path = "~/Documents/doc.txt";
    const char* path2 = strip_leading_tilde(path);
    ASSERT(eq(path2, TEST_HOME_DIR "/Documents/doc.txt"));

    path = "/home/yeah";
    path2 = strip_leading_tilde(path);
    ASSERT(eq(path, path2));
}
